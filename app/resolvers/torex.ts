...
export const TOREX_ADDRESS_MAP: Record<number, TOREXInfo[]> = {
  ...
  [arbitrum.id]: [
    ...
    {
      address: '0xd400a571ecac7fa44ed8f865be61a57f97290458' as Address,
      name: 'USDCx → ETHx TOREX',
    },
    {
      address: '0x7b4f1e8d184141261fce50aa496e0f8ee0007f76' as Address,
      name: 'ETHx → ARBx TOREX',
    },
    {
      address: '0x10fe08a38b21ac01e844620e393c2a13065eaa2a' as Address,
      name: 'ARBx → ETHx TOREX',
    },
    {
      address: '0x3109d8ee2e05c9251517fb5efd96e18d393c204b' as Address,
      name: 'ETHx → WBTCx TOREX',
    },
    {
      address: '0x16c10806853dfaea1fdbc42ba97e026993e15f81' as Address,
      name: 'WBTCx → ETHx TOREX',
    }
  ]
};
...
export const torexResolver: Resolver = {
  ...
  async getProfile(address) {
    try {
      const torexInfo = getTOREXInfo(address)
  ...
  async getAddress(handle) {
    try {
      const torexEntry = Object.values(TOREX_LOOKUP_MAP).find(
        torex => torex.name.toLowerCase() === handle.toLowerCase()
      )
      
      return torexEntry?.address || null
    } catch {
      return null
    }
  }
